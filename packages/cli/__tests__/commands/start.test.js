...
import { createTestWorkspace, runCli, readSession, getActiveState, listWorkflowStates, readWorkflowState, } from '../helpers/test-utils.js';
describe('start command', () => {
    ...
    describe('task queueing mode (--task)', () => {
        ...
        it('should queue task with workflow file', async () => {
            const result = runCli('start --task 1.1 runbooks/simple.runbook.md', workspace);
            expect(result.exitCode).toBe(0);
            expect(result.stdout).toContain('queued');
            expect(result.stdout).toContain('1.1');
            const state = await getActiveState(workspace);
            expect(state?.pendingTasks).toHaveLength(1);
            expect(state?.pendingTasks[0].workflow).toBe('runbooks/simple.runbook.md');
        });
    });
    describe('agent binding mode (--agent)', () => {
        ...
        it('sets agent status to running', async () => {
            runCli('start --agent test-agent', workspace);
            const state = await getActiveState(workspace);
            const binding = state?.agentBindings?.['test-agent'];
        ...
        it('should create child workflow linked to parent', async () => {
            ...
            runCli('start --task 1 runbooks/simple.runbook.md', workspace);
            ...
            const allStates = await Promise.all(stateFiles.map(file => readWorkflowState(workspace, file.replace('.json', ''))));
            const parentState = allStates.find(state => Object.values(state?.agentBindings || {})
                .some((binding) => typeof binding === 'object' &&
                binding !== null &&
                'childWorkflowId' in binding &&
                binding.childWorkflowId === session.active));
            expect(parentState).toBeTruthy();
            const agentBinding = parentState?.agentBindings?.['test-agent-123'];
            expect(agentBinding?.childWorkflowId).toBe(session.active);
        });
    });
});
